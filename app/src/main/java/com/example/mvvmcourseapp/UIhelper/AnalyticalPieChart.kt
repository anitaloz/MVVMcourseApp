...
import com.example.mvvmcourseapp.UIhelper.extension.dpToPx
import com.example.mvvmcourseapp.UIhelper.extension.draw
import com.example.mvvmcourseapp.UIhelper.extension.spToPx
import com.example.mvvmcourseapp.UIhelper.model.AnalyticalPieChartModel
import com.example.mvvmcourseapp.UIhelper.model.AnalyticalPieChartState
...
class AnalyticalPieChart @JvmOverloads constructor(
    ...
    init {
        ...
        if (attrs != null) {
            context.withStyledAttributes(attrs, R.styleable.AnalyticalPieChart) {
                val fontRes = getResourceId(R.styleable.AnalyticalPieChart_pieChartFontFamily, 0)
                if (fontRes != 0) {
                    fontFamily = ResourcesCompat.getFont(context, fontRes)
                }
                // Секция списка цветов
                val colorResId = getResourceId(R.styleable.AnalyticalPieChart_pieChartColors, 0)
                pieChartColors = resources.getStringArray(colorResId).toList()

                // Секция отступов
                marginTextFirst = getDimension(
                    R.styleable.AnalyticalPieChart_pieChartMarginTextFirst,
                    marginTextFirst
                )
                marginTextSecond = getDimension(
                    R.styleable.AnalyticalPieChart_pieChartMarginTextSecond,
                    marginTextSecond
                )
                marginTextThird = getDimension(
                    R.styleable.AnalyticalPieChart_pieChartMarginTextThird,
                    marginTextThird
                )
                marginSmallCircle = getDimension(
    ...
    override fun setDataChart(list: List<Pair<Int, String>>) {
        ...
        textRowList.clear()
        requestLayout() // ЗАПРАШИВАЕМ ПЕРЕРИСОВКУ РАЗМЕРОВ
        invalidate() // ЗАПРАШИВАЕМ ПЕРЕРИСОВКУ СОДЕРЖИМОГО
    }
    ...
    override fun startAnimation() {
        val animator = ValueAnimator.ofInt(0, 360).apply {
            duration = 1500 // Добавлено значение duration
    ...
    private fun drawCircle(canvas: Canvas) {
        ...
        for((index, percent) in percentageCircleList.withIndex()) {
            ...
            if (animationSweepAngle > percent.percentToStartAt + percent.percentOfCircle){
                ...
                val sweep = animationSweepAngle - percent.percentToStartAt
                canvas.drawArc(circleRect, percent.percentToStartAt, sweep, false, percent.paint)
        ...
        textRowList.forEachIndexed { index, staticLayout ->
            if (index % 2 == 0) {
                ...
                canvas.drawCircle(
                    ...
                    textCircleRadius,
                    Paint().apply { color =
                        pieChartColors[(index / 2) % pieChartColors.size].toColorInt() }
                )
    ...
    private fun initPains(textPaint: TextPaint, textSize: Float, textColor: Int, isDescription: Boolean = false) {
        ...
        // Установка шрифта для всех текстов
        fontFamily?.let {
            textPaint.typeface = it
        } ?: run {
            // Стандартное поведение, если шрифт не установлен
            if (!isDescription) {
    ...
    // Метод для программной установки шрифта
    fun setFontFamily(typeface: Typeface) {
        fontFamily = typeface
        // Обновляем все кисти
        amountTextPaint.typeface = typeface
        numberTextPaint.typeface = typeface
        descriptionTextPain.typeface = typeface
        invalidate()
    }

    // Метод для установки шрифта по resource ID
    ...
    private fun calculatePercentageOfData() {
        ...
        percentageCircleList = dataList.mapIndexed { index, pair ->
            ...
            val resultModel = AnalyticalPieChartModel(
                ...
                colorOfLine = pieChartColors[index % pieChartColors.size].toColorInt(),
    ...
    /**
     * Метод обертки текста в класс [android.graphics.Rect]
     */
