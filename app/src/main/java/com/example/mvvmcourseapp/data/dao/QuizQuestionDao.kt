...
interface QuizQuestionDao {
    ...
    @Query("""
        SELECT COUNT(*) as learned_questions 
        FROM quiz_questions qq 
        JOIN srs_tools srs ON qq.id = srs.qq_id 
        WHERE srs.interval >= :SRSinterval 
    ...
    @Query("""
        ...
        WHERE qq.category_id = :categoryId 
        AND c.category_name != "Тест на определение уровня"
        AND NOT EXISTS (
    ...
    @Query("""
        SELECT COUNT(*) as in_process_questions 
        FROM quiz_questions qq 
        JOIN srs_tools srs ON qq.id = srs.qq_id 
        WHERE srs.interval >= :SRSintervalDown 
        AND srs.interval < :SRSintervalUp 
        AND srs.user_id = :userID 
        AND category_id = :categoryId
    """)
    suspend fun getInProcessQuestionsByCategoryAndSRSinterval(
        categoryId: Int,
        SRSintervalDown: Int,
        SRSintervalUp: Int,
        userID: Int
    ...
    @Query("""
        ...
        AND c.lang_id = :langId
    """)
    suspend fun getInProcessQuestionsByLangAndSRSinterval(
        langId: Int,
        SRSintervalDown: Int,
        SRSintervalUp: Int,
        userID: Int
    ...
    """)
    suspend fun getQuizQuestionByCategoryAndLang(categoryID: Int): List<QuizQuestion>

    @Query("""
        SELECT q.* FROM quiz_questions q 
        JOIN categories c ON c.id = q.category_id
        LEFT JOIN srs_tools srs ON q.id = srs.qq_id AND srs.user_id = :userID 
        JOIN user_settings us ON us.user_id = :userID AND us.lang_id = c.lang_id
        WHERE q.category_id = :categoryID 
        AND c.lang_id = :langID  
        ...
        LIMIT :newQ
    """)
    suspend fun getQuizQuestionByCategoryAndLangAndSettingsAndNew(
        categoryID: Int,
        langID: Int,
        newQ: Int,
        userID: Int
    ...
    @Query("""
        ...
        LEFT JOIN srs_tools srs ON q.id = srs.qq_id AND srs.user_id = :userID
        WHERE q.category_id = :categoryID 
        AND (SELECT lang_id FROM categories WHERE id = q.category_id) = :langID
        AND srs.qq_id IS NULL 
        LIMIT :newQ
    """)
    suspend fun getQuizQuestionByCategoryAndLangAndNew(
        categoryID: Int,
        langID: Int,
        newQ: Int,
        userID: Int
    ...
    @Query("""
        ...
        LIMIT :maxRepQ
    """)
    suspend fun getQuizQuestionByCategoryAndLangAndCurDateAndSettings(
        categoryID: Int,
        langID: Int,
        userID: Int,
        maxRepQ: Int,
    ...
    @Query("""
        ...
        LIMIT :maxRepQ
    """)
    suspend fun getQuizQuestionByCategoryAndLangAndCurDate(
        categoryID: Int,
        langID: Int,
        userID: Int,
        maxRepQ: Int,
